...
namespace m_libnet
{
    ...
    TcpServer::TcpServer(EventLoop *loop,
                         ...
                         Option option)
        : loop_(CheckLoopNotNull(loop)), ipPort_(listenAddr.toIpPort()), name_(nameArg), acceptor_(new Accepter(loop, listenAddr, option == kReusePort)), threadPool_(new EventLoopThreadPool(loop, name_)), connectionCallback_(), messageCallback_(), nextConnId_(1), started_(0)
    {
    ...
    void TcpServer::start()
    {
        if (started_.fetch_add(1) == 0) // 防止一个TcpServer对象被start多次
        {
            ...
            loop_->runInLoop(std::bind(&Accepter::listen, acceptor_.get()));
        }
    }
